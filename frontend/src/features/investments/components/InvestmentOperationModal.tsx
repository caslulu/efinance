...
export function InvestmentOperationModal({
  ...
  const shouldLoadQuote = isOpen && normalizedSymbol.length > 0;
  const {
    data: quoteResults = [],
      ...
      <DialogContent className="sm:max-w-[520px]">
        ...
        {investmentWallets.length === 0 ? (
          ...
          <form className="grid gap-4" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="operation-symbol">Ticker</Label>
                <Input
                  id="operation-symbol"
                  value={symbol}
                  onChange={(event) => setSymbol(event.target.value.toUpperCase())}
                  placeholder={market === 'BR' ? 'PETR4' : 'TSLA'}
                />
              </div>
            ...
            <div className="rounded-xl border border-border/60 bg-muted/30 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-foreground">Cotação usada no lançamento</p>
                  <p className="text-xs text-muted-foreground">
                    A operação será registrada com o preço atual do ativo no momento do envio.
                  </p>
                </div>
                {isQuoteLoading && normalizedSymbol ? (
